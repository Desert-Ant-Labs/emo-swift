...
// in the repo-root mise.toml) builds libEmoAndroid.so per ABI, copies it +
// libc++_shared.so + libLiteRt.so into src/main/jniLibs, and stages the model
// into the optional :emo-tflite-resources module's resources. This task runs
...
val buildSwiftNatives by tasks.registering(Exec::class) {
    ...
    outputs.dir("$rootDir/emo-tflite-resources/src/main/resources")
}
