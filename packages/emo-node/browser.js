...
// resolves model assets, owns the LiteRT.js session, and exposes the public
// typed API (an `Emo` class with an async `load` factory). It runs in the
// browser and, via the platform seam below, server-side in Node (the
...
function assertRuntimeCanLoadLiteRt(options) {
  ...
  throw new Error(
    "@desert-ant-labs/emo: the default import runs the browser WebAssembly " +
      ...
      "build instead: import { Emo } from \"@desert-ant-labs/emo/native\". " +
      "(The default import is still safe to bundle for server-side rendering; " +
      "only calling Emo.load() in Node needs the native build.)",
  );
}
